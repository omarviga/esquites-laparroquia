...
export function RecipeDialog({
  ...
  const { data: recipes } = useQuery({
    ...
    queryFn: () =>
      productId
        ? inventoryApi.getRecipesEnriched(productId)
        : Promise.resolve([]),
    enabled: !!productId,
  });
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const currentIds = localItems
        .filter((l) => l.id)
        .map((l) => l.id);
      const toDelete =
        recipes?.filter((r) => !currentIds.includes(r.id)) || [];
      ...
      <DialogContent className="bg-card gold-border max-w-xl">
        ...
        <div className="space-y-4 py-4">
          <p className="text-sm text-muted-foreground">
            Define los insumos y la cantidad exacta que se consume por cada
