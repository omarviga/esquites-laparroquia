...
export const getSalesHistory = async (data: z.infer<typeof historyInput>) => {
  const { supabase } = await getAuthContext();
  const { page, pageSize, dateFrom, dateTo, paymentMethod, status, search, sortBy, sortOrder } = data;
...
export const getSaleDetail = async (data: z.infer<typeof detailInput>) => {
  const { supabase } = await getAuthContext();
...
export const getSalesSummary = async (data: z.infer<typeof summaryInput>) => {
  const { supabase } = await getAuthContext();
...
export const cancelSaleFn = async (data: z.infer<typeof cancelInput>) => {
