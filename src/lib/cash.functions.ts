...
export const openCashRegister = async (data: z.infer<typeof openInput>) => {
...
export const closeCashRegister = async (data: z.infer<typeof closeInput>) => {
  const { supabase, userId } = await getAuthContext();
...
export const addCashMovement = async (data: z.infer<typeof moveInput>) => {
  const { supabase, userId } = await getAuthContext();
  const { data: reg } = await supabase
    .from("cash_register")
    .select("id")
...
export const getCashCutDetail = async (data: z.infer<typeof cutDetailInput>) => {
  const { supabase } = await getAuthContext();
