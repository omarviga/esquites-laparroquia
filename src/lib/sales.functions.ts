...
export const saveSale = async (data: z.infer<typeof saveSaleInput>) => {
...
export const getSaleForTicket = async (data: z.infer<typeof getSaleInput>) => {
...
export const updateKdsStatus = async (data: { saleId: string; status: string }) => {
  const { supabase } = await getAuthContext();
