...
export const uploadMenu = async (data: z.infer<typeof uploadInput>) => {
  const { supabase, userId } = await getAuthContext();
  await assertAdmin(supabase, userId);
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const setActiveMenu = async (data: { id: string }) => {
  const { supabase, userId } = await getAuthContext();
  await assertAdmin(supabase, userId);
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const deleteMenu = async (data: { id: string }) => {
  const { supabase, userId } = await getAuthContext();
  await assertAdmin(supabase, userId);
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const getMenuSignedUrl = async (data: { id: string }) => {
  const { supabase, userId } = await getAuthContext();
  await assertAdmin(supabase, userId);
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data: menu } = await supabaseAdmin.from("digital_menus").select("file_url").eq("id", data.id).maybeSingle();
...
export const getPublicMenuUrl = async (data?: { id?: string }) => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  let menu;
  if (data?.id) {
